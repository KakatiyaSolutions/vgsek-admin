...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['image'])) {
    ...
    if ($_FILES['image']['error'] == 0) {
        ...
        if ($id) {
            $stmt = $con->prepare("UPDATE contact_hod SET name=?, image=?, qualification=?, designation=?, department=?, message=?, phone=?, email=?, department_id=? WHERE id=?");
            $null = NULL;
            $stmt->bind_param("sbssssssii", $name, $null, $qualification, $designation, $department,$message,$phone,$email, $department_id,$id);
            $stmt->send_long_data(1, $imageData);
        } else {
            $stmt = $con->prepare("INSERT INTO contact_hod (name, image, qualification, designation, department, department_id,message ,phone, email) VALUES (?, ?, ?, ?,?,?,?,?,?)");
            $null = NULL;
...
if (isset($_GET['id'])) {
    $id = (int) $_GET['id'];
    $query = "SELECT * FROM contact_hod WHERE id = ?";
...
if (isset($_GET['delete'])) {
    $id = (int) $_GET['delete'];
    $query = "DELETE FROM contact_hod WHERE id = ?";
    $stmt = $con->prepare($query);
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
        exit;
...
$query = "SELECT * FROM contact_hod $searchQuery LIMIT $itemsPerPage OFFSET $offset";
...
$totalQuery = "SELECT COUNT(*) as total FROM contact_hod $searchQuery";
...
<body>
    ...
    <script>
        function editFaculty(id) {
            fetch(`update_contact_hod.php?id=${id}`)
