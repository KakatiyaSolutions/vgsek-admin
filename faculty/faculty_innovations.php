...
$department = isset($_GET['department']) ? $_GET['department'] : 'MEC'; // Default to 'CSE'

// Fetch data for the selected department
$query = "SELECT sno, department,department_name, description, status, vision, mission, peo,poso
          FROM dep_faculty_innovations WHERE department_name = ?";
...
$department = $department_name = $vision = $mission = $description = $poso = $peo ="";
if ($result->num_rows > 0) {
    ...
    $poso = htmlspecialchars_decode($row['poso']);
}
// $peo_lines = explode("\n", $peo);  // Split the string by newline
// $peo_header = $peo_lines[1];  
$stmt->close();
mysqli_close($con);
?>
...
<div class="main-content">
            ...
            <h1>Update Faculty Innovations - Department Details of <?php echo htmlspecialchars($department_name); ?></h1>
             <form action="update_faculty_innovations.php?department=<?php echo urlencode($department_name); ?>" method="POST">
        <label for="description">Academic Year 2022-23:</label><br>
        <textarea name="description" id="description"><?php echo $description; ?></textarea><br><br>

        <label for="vision">Academic Year 2021-22:</label><br>
        <textarea name="vision" id="vision"><?php echo $vision; ?></textarea><br><br>

        <label for="mission">Academic Year 2020-21:</label><br>
        <textarea name="mission" id="mission"><?php echo $mission; ?></textarea><br><br>
<!--  <?php echo ($peo_header) ?>-->
        <label for="peo">List of Pedagogical Initiatives by the Faculty Members:</label><br>
        <textarea name="peo" id="peo"><?php echo $peo; ?></textarea><br><br>
        
        <label for="poso"> ECE Faculty YouTube Lecture Videos:</label><br>
        <textarea name="poso" id="poso"><?php echo $poso; ?></textarea><br><br>
...
<script>
    ...
    const editorIds = 
    ['description', 'vision', 'mission', 'peo', 'poso'];
